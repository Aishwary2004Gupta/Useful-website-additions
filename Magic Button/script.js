...
const backgrounds = [
  "linear-gradient(45deg, #16213e, #0f3460)", // Pink gradient
  "linear-gradient(45deg, #1a2a6c, #fdbb2d)", // Blue-yellow gradient
  "linear-gradient(45deg,rgb(200, 178, 178),rgb(39, 19, 14))", // Neon blue gradient
];
...
// Default neon line animation
function neonTrail() {
  particles.push(new Particle(mouse.x, mouse.y, 200, 'circle'));
}

// Trigger animations based on the current animation
function triggerAnimation() {
  if (currentAnimation === 0) {
    createParticles(10, 'alphabet');
  } else if (currentAnimation === 1) {
    createParticles(20, 'shape');
  } else {
    neonTrail();
  }
}
